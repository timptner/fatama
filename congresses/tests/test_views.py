...
class AttendanceCreateViewTest(TestCase):
    def setUp(self) -> None:
        ...
        self.congress = Congress.objects.create(title="Tagung", location="Magdeburg", year=2024)
        self.path = reverse('congresses:create_attendance', kwargs={'year': self.congress.year})
    ...
    def test_user_form_view(self):
        self.client.force_login(self.user)
        response = self.client.post(self.path)
        self.assertRedirects(response, reverse('congresses:congress_detail', kwargs={'year': self.congress.year}))
...
class AttendanceDetailViewTest(TestCase):
    def setUp(self) -> None:
        ...
        self.path = reverse('congresses:attendance_detail', kwargs={'pk': self.attendance.pk})
...
class CongressDetailViewTest(TestCase):
    def setUp(self) -> None:
        ...
        self.path = reverse('congresses:congress_detail', kwargs={'year': self.congress.year})
        self.user = User.objects.create_user(username='john')
...
class ParticipantCreateViewTest(TestCase):
    def setUp(self) -> None:
        ...
        congress = Congress.objects.create(title="Tagung", location="Magdeburg", year=2024)
        self.attendance = Attendance.objects.create(congress=congress, council=council)
        self.path = reverse('congresses:create_participant', kwargs={'pk': self.attendance.pk})
    ...
    def test_user_form_view(self) -> None:
        ...
        path = reverse('congresses:attendance_detail', kwargs={'pk': self.attendance.pk})
...
class PortraitCreateViewTest(TestCase):
    def setUp(self) -> None:
        ...
        self.path = reverse('congresses:create_portrait', kwargs={'pk': self.participant.pk})
